...
const customerImages = [
  { src: "https://stoneworksremodlling.s3.ap-south-1.amazonaws.com/webp/profile/user1.webp", alt: "Satisfied bathroom remodeling customer Livonia Metro Detroit" },
  { src: "https://stoneworksremodlling.s3.ap-south-1.amazonaws.com/webp/profile/user2.webp", alt: "Walk-in tub customer Rochester Metro Detroit" },
  { src: "https://stoneworksremodlling.s3.ap-south-1.amazonaws.com/webp/profile/user3.webp", alt: "Shower conversion customer Detroit Metro Detroit" },
  { src: "https://stoneworksremodlling.s3.ap-south-1.amazonaws.com/webp/profile/user4.webp", alt: "Walk-in tub customer Troy Metro Detroit" },
];
...
// FIX: Service cards link to their actual service pages
const services = [
...
// ─── Schema JSON-LD — FAQPage only (LocalBusiness is in layout.tsx) ──────────
...
function QuoteForm() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    // Build a clean, readable summary for Val so she knows exactly
    // what this lead is about the moment she reads the email
    const leadSummary = [
      ...
      const res = await fetch(
        "https://dl4ltl1h1f.execute-api.us-east-1.amazonaws.com/api/submit",
        {
          ...
          body: JSON.stringify({
            ...
            address: form.zip,
            // Put the readable summary where Val will see it clearly
            designStyle: form.service,
            materialPreferences: leadSummary,
            removalNeeded: form.message || "Customer will discuss on call",
            // Required fields Lambda validates — filled with clear values
  ...
  const inputCls = "w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-100 outline-none text-sm transition";
    ...
    <form onSubmit={onSubmit} className="space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input type="text" name="name" required value={form.name} onChange={onChange} placeholder="John Smith" className={inputCls} />
        </div>
        <div>
          <label className="block text-sm font-semibold text-gray-700 mb-1">Phone Number *</label>
          <input type="tel" name="phone" required value={form.phone} onChange={onChange} placeholder="(248) 555-0100" className={inputCls} />
        </div>
      </div>
      <div>
        <label className="block text-sm font-semibold text-gray-700 mb-1">Email Address</label>
        <input type="email" name="email" value={form.email} onChange={onChange} placeholder="john@email.com" className={inputCls} />
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-semibold text-gray-700 mb-1">Service Needed *</label>
          <select name="service" required value={form.service} onChange={onChange} className={inputCls + " bg-white"}>
        ...
        <div>
          ...
          <input type="text" name="zip" required value={form.zip} onChange={onChange} placeholder="48150" className={inputCls} />
        </div>
      </div>
      <div>
        <label className="block text-sm font-semibold text-gray-700 mb-1">Tell us about your project (optional)</label>
        <textarea name="message" value={form.message} onChange={onChange} placeholder="e.g. Small master bath, need tub converted to shower, budget around $10k..." rows={3} className={inputCls + " resize-none"} />
      </div>
      {status === "error" && <p className="text-red-600 text-sm">Something went wrong. Please call us directly at {PHONE}.</p>}
      <button type="submit" disabled={status === "submitting"} className="w-full bg-blue-600 hover:bg-blue-700 disabled:opacity-60 text-white py-4 rounded-xl font-bold text-base transition flex items-center justify-center gap-2">
        {status === "submitting"
...
export default function HomePage() {
  ...
  return (
    <>
      {/* FAQPage schema — page-specific. LocalBusiness schema is in layout.tsx */}
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(faqSchema) }} />

      <div className="min-h-screen bg-white overflow-x-hidden">

        {/* ── Mobile sticky CTA (replaces rotating button) ── */}
        <div className="fixed bottom-0 left-0 right-0 z-50 md:hidden bg-white border-t border-slate-200 px-4 py-3 flex gap-3">
          <a href={PHONE_HREF} onClick={onCallClick} className="flex-1 bg-blue-600 text-white py-3 rounded-xl font-bold text-sm flex items-center justify-center gap-2">
            <Phone className="h-4 w-4" />Call Now
          </a>
          <a href="#quote-form" className="flex-1 border-2 border-blue-600 text-blue-600 py-3 rounded-xl font-bold text-sm flex items-center justify-center gap-2">
            <MessageCircle className="h-4 w-4" />Free Quote
          </a>
        </div>

        {/* ── Desktop floating CTA ── */}
        <div className="fixed bottom-6 right-6 z-50 hidden md:block">
          <a href="#quote-form" className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-3 rounded-full shadow-xl transition flex items-center gap-2 font-bold text-sm">
        ...
        <main>
          ...
          <section className="relative bg-gray-900 py-12 sm:py-16 lg:py-24 overflow-hidden">
            <div className="absolute inset-0 z-0 bg-[radial-gradient(ellipse_at_30%_70%,rgba(37,99,235,0.25),transparent_60%)]" />
            <div className="container mx-auto px-4 relative z-10">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 items-center">

                {/* Text */}
                <motion.div className="text-white text-center lg:text-left" initial={{ opacity: 0, y: shouldReduceMotion ? 0 : 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45 }}>
                  <span className="inline-flex items-center gap-2 bg-blue-600 text-white px-4 py-1.5 rounded-full text-xs font-bold mb-5 uppercase tracking-wide">
                    <Star className="h-3.5 w-3.5 fill-yellow-400 text-yellow-400" />#1 Rated Bathroom Remodeler — Metro Detroit
                  </span>
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 mb-10 justify-center lg:justify-start">
                    <a href={PHONE_HREF} onClick={onCallClick} className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-4 rounded-xl font-bold text-base transition shadow-xl flex items-center justify-center gap-2 w-full sm:w-auto">
                      <Phone className="h-5 w-5" />{PHONE}
                    </a>
                    <a href="#quote-form" className="border-2 border-white/40 hover:border-white/80 text-white px-8 py-4 rounded-xl font-bold text-base transition flex items-center justify-center gap-2 w-full sm:w-auto">
                      <Calendar className="h-5 w-5" />Schedule Free Consultation
                    </a>
                  </div>
                  {/* Social proof */}
                  <div className="flex items-center justify-center lg:justify-start gap-4">
                    <div className="flex -space-x-2">
                      {customerImages.map((img, i) => (
                        <div key={img.src} className="w-10 h-10 rounded-full border-2 border-white overflow-hidden bg-blue-300">
                          {/* FIX: only first 2 priority, no conflicting fetchPriority */}
                ...
                {/* Offer Card with LIVE countdown */}
                <motion.div className="bg-white/10 backdrop-blur border border-white/20 rounded-3xl p-7 sm:p-10 max-w-md mx-auto w-full order-first lg:order-last" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.15 }}>
                  <div className="bg-blue-600 -mx-7 sm:-mx-10 -mt-7 sm:-mt-10 mb-6 px-6 py-4 rounded-t-3xl text-center">
                    <p className="text-blue-100 text-xs font-bold uppercase tracking-widest flex items-center justify-center gap-1.5">
                      <Zap className="h-3.5 w-3.5" />Limited Time Offer — Metro Detroit
                    </p>
                  </div>
                      ...
                      <div className="grid grid-cols-4 gap-2">
                        {[{ val: countdown.days, label: "Days" }, { val: countdown.hours, label: "Hrs" }, { val: countdown.mins, label: "Min" }, { val: countdown.secs, label: "Sec" }].map(({ val, label }) => (
                          <div key={label} className="bg-white/10 rounded-xl py-2">
                            <p className="text-white text-2xl font-black leading-none">{String(val).padStart(2, "0")}</p>
                            <p className="text-blue-300 text-xs mt-1">{label}</p>
                          </div>
                        ))}
                      </div>
                    </div>
                    <div className="bg-red-600 text-white rounded-xl px-4 py-2.5 font-bold text-sm mb-5">Only 2 spots left this month!</div>
                    <p className="text-xs text-blue-300 italic mb-5">*Valid for Wayne, Oakland &amp; Macomb Counties</p>
                    <a href={PHONE_HREF} onClick={onCallClick} className="block w-full bg-green-500 hover:bg-green-600 text-white py-4 rounded-xl font-bold text-base transition text-center">
                      <Phone className="inline h-5 w-5 mr-2" />Call to Claim This Offer
                    </a>
          ...
          {/* ── FIX: Trust badge strip below hero ── */}
              ...
              <div className="flex flex-wrap items-center justify-center gap-x-8 gap-y-3">
                {[
                  ...
                  <div key={text} className="flex items-center gap-2 text-slate-700 text-sm font-semibold">
                    <Icon className="h-4 w-4 text-blue-600 flex-shrink-0" />{text}
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="container mx-auto px-4">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 sm:gap-6">
                {stats.map((s, i) => {
                  const Icon = s.icon;
                  return (
                    <motion.div key={s.label} className="text-center bg-white rounded-2xl p-5 sm:p-6 shadow-sm border border-slate-100" {...inView} transition={{ delay: i * 0.08, duration: 0.4, ease: "easeOut" }}>
                      <div className="w-12 h-12 bg-blue-600 rounded-xl flex items-center justify-center mx-auto mb-3"><Icon className="h-6 w-6 text-white" /></div>
            ...
            <div className="container mx-auto px-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {services.map((service, i) => {
                  ...
                  return (
                    <motion.div key={service.title} className="bg-white rounded-2xl overflow-hidden shadow-sm border border-slate-100 hover:shadow-lg hover:-translate-y-1 transition-all duration-300 group" {...inView} transition={{ delay: i * 0.08, duration: 0.4, ease: "easeOut" }}>
                      <div className="relative h-44 overflow-hidden">
                        {/* FIX: only first image eager, rest lazy, no conflicting fetchPriority */}
                        <Image src={service.image} alt={service.alt} fill className="object-cover group-hover:scale-105 transition-transform duration-500" sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 25vw" loading={i === 0 ? "eager" : "lazy"} quality={75} />
                      </div>
                      <div className="p-5">
                        <div className="flex items-center gap-2 mb-2"><Icon className="h-5 w-5 text-blue-600" /><h3 className="text-base font-bold text-gray-900">{service.title}</h3></div>
                        ...
                        <ul className="space-y-1.5 mb-4">
                          {["Free in-home consultation", "Licensed MI contractors", "5-year warranty"].map((item) => (
                            <li key={item} className="flex items-center gap-2 text-xs text-slate-600"><Check className="h-3.5 w-3.5 text-green-500 flex-shrink-0" />{item}</li>
                          ))}
                        </ul>
                        {/* FIX: links to actual service page */}
              ...
              <div className="mt-12 text-center">
                <a href={PHONE_HREF} onClick={onCallClick} className="inline-flex items-center bg-blue-600 hover:bg-blue-700 text-white px-8 py-4 rounded-xl font-bold transition shadow-lg gap-2">
                  <Phone className="h-5 w-5" />Call {PHONE} for Free Consultation
                </a>
              </div>
            </div>
          </section>
            ...
            <div className="container mx-auto px-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {processSteps.map((item, i) => (
                  <motion.div key={item.title} className="bg-white rounded-2xl p-7 border border-slate-100 shadow-sm h-full relative" {...inView} transition={{ delay: i * 0.08, duration: 0.4, ease: "easeOut" }}>
                    <div className="w-12 h-12 bg-blue-600 rounded-xl flex items-center justify-center font-black text-lg text-white mb-5 mx-auto">{item.step}</div>
                    ...
                    {i < 3 && <div className="hidden lg:block absolute top-1/2 -right-3 w-6 h-0.5 bg-blue-200" />}
                  </motion.div>
                ))}
              </div>
              <div className="mt-10 text-center">
                <a href="#quote-form" className="inline-flex items-center bg-blue-600 hover:bg-blue-700 text-white px-8 py-4 rounded-xl font-bold transition shadow-lg gap-2">
          ...
          {/* ══ GALLERY — FIX: No Play icon, proper links, View Project overlay ══ */}
            ...
            <div className="container mx-auto px-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                {recentProjects.map((project, i) => (
                  <motion.div key={i} className="group rounded-2xl overflow-hidden shadow-sm border border-slate-100 hover:shadow-lg transition-all duration-300" {...inView} transition={{ delay: i * 0.08, duration: 0.4, ease: "easeOut" }}>
                    <Link href={project.link}>
                      <div className="relative h-56 overflow-hidden">
                        <Image src={project.src} alt={project.alt} fill className="object-cover group-hover:scale-105 transition-transform duration-500" sizes="(max-width: 640px) 100vw, 33vw" loading="lazy" quality={75} />
                        {/* FIX: Replaced fake Play icon with View Service overlay */}
                        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                          <span className="bg-white text-gray-900 px-4 py-2 rounded-xl font-bold text-sm flex items-center gap-1.5">View Service<ExternalLink className="h-4 w-4" /></span>
                        </div>
                      </div>
                      <div className="p-5 bg-white">
                        <h3 className="font-bold text-gray-900 text-sm sm:text-base mb-1">{project.title}</h3>
                        <p className="text-slate-500 text-xs sm:text-sm flex items-center gap-1"><MapPin className="h-3.5 w-3.5 text-blue-600" />{project.location}, Metro Detroit</p>
                      </div>
                    </Link>
                  </motion.div>
                ))}
              ...
              <div className="mt-10 text-center">
                <Link href="/gallery" className="inline-flex items-center border-2 border-blue-600 text-blue-600 hover:bg-blue-600 hover:text-white px-8 py-4 rounded-xl font-bold transition gap-2">
          ...
          {/* ══ TESTIMONIALS — FIX: All 3 shown on desktop ══ */}
          <section className="py-16 sm:py-20 bg-slate-50">
            <div className="container mx-auto px-4">
              <motion.div className="text-center mb-12" {...inView}>
                <div className="inline-flex items-center gap-2 bg-white border border-slate-200 rounded-full px-5 py-2 mb-5 shadow-sm">
                  <div className="flex items-center gap-0.5">{[1,2,3,4,5].map((n) => <Star key={n} className="h-4 w-4 text-yellow-400 fill-yellow-400" />)}</div>
              ...
              <div className="hidden md:grid md:grid-cols-3 gap-6 mb-8">
                {testimonials.map((t) => (
                  <div key={t.id} className="bg-white rounded-2xl p-7 shadow-sm border border-slate-100 flex flex-col">
                    <div className="flex items-center gap-0.5 mb-3">{[...Array(t.rating)].map((_, i) => <Star key={i} className="h-4 w-4 text-yellow-400 fill-yellow-400" />)}</div>
                    <p className="text-slate-700 text-sm leading-relaxed italic mb-5 flex-1">&ldquo;{t.quote}&rdquo;</p>
                    <div className="flex items-center gap-3">
                      <Image src={t.image} alt={t.alt} width={44} height={44} className="rounded-full object-cover border-2 border-blue-100" loading="lazy" quality={70} />
                      <div>
                        <p className="font-bold text-gray-900 text-sm">{t.name}</p>
                        <p className="text-xs text-slate-500 flex items-center gap-1"><MapPin className="h-3 w-3 text-blue-600" />{t.location}</p>
                      </div>
                    </div>
                    <div className="mt-3 pt-3 border-t border-slate-100">
                      <span className="text-xs text-blue-600 font-semibold bg-blue-50 px-3 py-1 rounded-full">{t.projectType}</span>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="md:hidden">
                <AnimatePresence mode="wait">
                  <motion.div key={activeTestimonial} className="bg-white rounded-2xl p-7 shadow-sm border border-slate-100" initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.35 }}>
                    <div className="flex items-center gap-0.5 mb-3">{[...Array(testimonials[activeTestimonial].rating)].map((_, i) => <Star key={i} className="h-4 w-4 text-yellow-400 fill-yellow-400" />)}</div>
                    <p className="text-slate-700 italic leading-relaxed mb-5">&ldquo;{testimonials[activeTestimonial].quote}&rdquo;</p>
                    <div className="flex items-center gap-3">
                      <Image src={testimonials[activeTestimonial].image} alt={testimonials[activeTestimonial].alt} width={44} height={44} className="rounded-full object-cover" loading="lazy" quality={70} />
                      <div><p className="font-bold text-gray-900">{testimonials[activeTestimonial].name}</p><p className="text-xs text-slate-500">{testimonials[activeTestimonial].location}</p></div>
                    </div>
                  </motion.div>
                </AnimatePresence>
                <div className="flex justify-center gap-2 mt-5">
                  {testimonials.map((_, i) => (
                    <button key={i} onClick={() => setActiveTestimonial(i)} className={`h-2.5 rounded-full transition-all ${activeTestimonial === i ? "w-6 bg-blue-600" : "w-2.5 bg-slate-300"}`} aria-label={`Show testimonial ${i + 1}`} />
                  ))}
                </div>
              </div>
              ...
              <div className="mt-10 text-center">
                <a href={PHONE_HREF} onClick={onCallClick} className="inline-flex items-center bg-blue-600 hover:bg-blue-700 text-white px-8 py-4 rounded-xl font-bold transition shadow-lg gap-2">
                  <Phone className="h-5 w-5" />Join Our Happy Customers
                </a>
              </div>
            </div>
          </section>

          {/* ══ INLINE QUOTE FORM — NEW SECTION ══ */}
            ...
            <div className="container mx-auto px-4">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-start max-w-5xl mx-auto">
                <div className="text-white">
                  ...
                  <p className="text-blue-100 text-lg mb-8 leading-relaxed">Fill out the form and we&apos;ll call you within 2 business hours to schedule your free consultation. No pressure, no obligation.</p>
                  <div className="space-y-4">
                    {["Free in-home measurement & consultation", "Detailed, itemized cost breakdown", "3D design preview before work starts", "$2,500 OFF applied if you book this month", "5-year workmanship warranty included"].map((item) => (
                      <div key={item} className="flex items-center gap-3">
                        <div className="w-6 h-6 rounded-full bg-green-400 flex items-center justify-center flex-shrink-0"><Check className="h-3.5 w-3.5 text-white" /></div>
                  ...
                  <div className="mt-8 pt-8 border-t border-blue-500">
                    ...
                    <a href={PHONE_HREF} onClick={onCallClick} className="inline-flex items-center gap-2 text-white font-bold text-lg hover:text-blue-200 transition"><Phone className="h-5 w-5" />{PHONE}</a>
                    <p className="text-blue-200 text-xs mt-1">Mon–Fri 8am–6pm · Sat 9am–2pm</p>
                  </div>
                </div>
                <div className="bg-white rounded-2xl p-7 sm:p-8 shadow-xl"><QuoteForm /></div>
              </div>
            </div>
          </section>

          {/* ══ FAQ — FIX: JSON-LD schema + always in DOM ══ */}
          <section className="py-16 sm:py-20 bg-white">
            <div className="container mx-auto px-4">
              <motion.div className="text-center mb-12" {...inView}>
              ...
              <div className="max-w-3xl mx-auto space-y-3">
                {faqs.map((faq, i) => (
                  <div key={i} className="border border-slate-200 rounded-2xl overflow-hidden hover:border-blue-300 transition-colors">
                    <button className="w-full text-left flex justify-between items-center py-5 px-6 hover:bg-slate-50 transition-colors" onClick={() => setActiveFaq(activeFaq === i ? null : i)} aria-expanded={activeFaq === i}>
                      <h3 className="text-base font-bold text-gray-900 pr-4">{faq.question}</h3>
                      <ChevronDown className={`h-5 w-5 text-blue-600 flex-shrink-0 transition-transform ${activeFaq === i ? "rotate-180" : ""}`} />
                    </button>
                    {/* FIX: always in DOM, CSS height transition, Google can index it */}
                    <div className={`px-6 overflow-hidden transition-all duration-300 ${activeFaq === i ? "max-h-48 pb-5 opacity-100" : "max-h-0 opacity-0"}`} aria-hidden={activeFaq !== i}>
            ...
            <div className="container mx-auto px-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-5xl mx-auto">
                {whyChooseFeatures.map((f, i) => {
                  const Icon = f.icon;
                  return (
                    <motion.div key={f.title} className="bg-white rounded-2xl p-7 border border-slate-100 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all" {...inView} transition={{ delay: i * 0.07, duration: 0.4, ease: "easeOut" }}>
                      <div className="w-12 h-12 bg-blue-600 rounded-xl flex items-center justify-center mb-4"><Icon className="h-6 w-6 text-white" /></div>
          ...
          <section className="py-14 bg-white border-t border-slate-100">
            <div className="container mx-auto px-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                {[
                  { src: "/instagram/image.png", href: "https://www.instagram.com/reel/DNQxTAGOaTg/", alt: "Bathroom remodeling project Metro Detroit" },
                  ...
                  <a key={item.href} href={item.href} target="_blank" rel="noopener noreferrer" className="block rounded-2xl overflow-hidden shadow-sm border border-slate-100 group hover:shadow-md transition">
                    <div className="relative h-64 overflow-hidden">
                      <Image src={item.src} alt={item.alt} fill className="object-cover group-hover:scale-105 transition-transform duration-500" sizes="(max-width: 640px) 100vw, 33vw" loading="lazy" quality={70} />
                    </div>
                  </a>
                ))}
              </div>
              <div className="mt-6 text-center flex justify-center gap-4">
                <a href="https://www.facebook.com/people/Stone-Works-Remodeling/61567020355631/" target="_blank" rel="noopener noreferrer" className="inline-flex items-center bg-blue-700 text-white px-5 py-2.5 rounded-xl font-bold text-sm hover:bg-blue-800 transition gap-2">
                  <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>Facebook
                </a>
                <a href="https://www.youtube.com/@stoneworksremodeling" target="_blank" rel="noopener noreferrer" className="inline-flex items-center bg-red-600 text-white px-5 py-2.5 rounded-xl font-bold text-sm hover:bg-red-700 transition gap-2">
            ...
            <div className="container mx-auto px-4">
              <div className="text-center mb-6">
                <h2 className="text-2xl font-bold text-gray-900 flex items-center justify-center gap-2 mb-2">
                  <MapPin className="h-5 w-5 text-blue-600" />Serving All of Metro Detroit
                </h2>
                <p className="text-slate-500 text-sm">Wayne County · Oakland County · Macomb County</p>
              </div>
              <div className="flex flex-wrap justify-center gap-2">
                {serviceAreas.map((city) => (
                  <Link key={city} href={`/${city.toLowerCase().replace(/\s+/g, "-")}`} className="bg-slate-50 border border-slate-200 hover:border-blue-400 hover:bg-blue-50 hover:text-blue-700 text-slate-700 px-3 py-1.5 rounded-full text-sm transition" aria-label={`Bathroom remodeling in ${city}, MI`}>
                    {city}
                  </Link>
                ))}
              </div>
